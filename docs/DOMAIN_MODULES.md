...
- Components: `RecurrenceCard` (uses `suppressHydrationWarning` on locale-formatted dates), `RecurrenceModal` (visual clone of TaskModal with frequency controls + `onToast` for category delete feedback), `AbsenceModal` (role-aware: members see static own-name field, admins see dropdown to pick any member)
